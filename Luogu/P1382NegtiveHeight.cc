...
void pushUp(int pos,int l,int r){
    if(tr[pos].cvT){
        ...
        tr[pos].yD=min(tr[pos<<1].yD,tr[pos<<1|1].yD);
        tr[pos].yU=max(tr[pos<<1].yU,tr[pos<<1|1].yU);
...
int main(){
    ...
    for(int i= 1;i<=n;i++){
        ...
        if(tr[1].yD!=lastD){
            ans.push_back((ANS){line[i].x,lastD});
            ans.push_back((ANS){line[i].x,tr[1].yD});
        }
        if(tr[1].yU!=lastU){
            ans.push_back((ANS){line[i].x,lastU});
            ans.push_back((ANS){line[i].x,tr[1].yU});
        }
        lastU=tr[1].yU,lastD=tr[1].yD;
    }
